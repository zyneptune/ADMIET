...
# 生成自变量
if not os.path.exists("./data/datax_4.txt"):
    x = np.random.rand(100, 1) * 4.0
    np.savetxt("data/datax_4.txt", x)
if not os.path.exists("./data/parameters_4.txt"):
    p = np.random.randn(100, 2)*2.0
    np.savetxt("./data/parameters_4.txt", p)

x = np.loadtxt("data/datax_4.txt")
x = x.reshape([100,1])
p = np.loadtxt("./data/parameters_4.txt")
# 生成数据并计算
path_exe = "../../build/functionfitting"
path_datax = " --DataX=./data/datax_4.txt"
# path_datay = " --DataY=./data/datay_1.txt"
bound = " --ConstantUpperBound=10.0" + " --ConstantLowerBound=-10.0"
Num_nodes = " --NumNodes=7"
Settings = " --StepLength=0.03 --Epsilon=10.0 --MaxEpoch=3000 --WhenPenalty=1000"
result = []
for i in range(100):
    y = func(x, p[i, :])
    np.savetxt("./data/datay_4_"+str(i+1)+".txt", y)
    path_datay = " --DataY=./data/datay_4_"+str(i+1)+".txt"
    temp_results = []
    ...
    for j in temp_results:
        with open('result/result_4_all.txt', 'a') as f:
            print(j, file=f)
...
re = np.array(result)
np.savetxt("result/result_4.txt", re)
